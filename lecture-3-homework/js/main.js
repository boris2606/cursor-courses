...
document.querySelector('.result_sum').innerHTML = `Сума чисал становить <span class="colored">${sumNum}</span> при обранні початкового значення: <span class="colored">${startNumber}</span> кінцевого значення: <span class="colored">${lastNumber}</span> (Статус пропуску парних чисел <span class="colored">"${confirmRequest}</span>")`
